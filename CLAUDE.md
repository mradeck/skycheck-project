...
**Aktuell:** v26.08.112.0 — **DE-Höhengitter** (zuschaltbar): Ampel-Heatmap der max. Flughöhe je Rasterzelle in DFS-CTR 2–4 (Vorbild: tschechisches DroneMap-GRID). Hybrid — DiPUL-WMS zeigt die Sub-Zonen-Grenzen, das Overlay färbt ein Open-Meteo-Gelände-Raster via `computeCtrFlightInfo` mit der am Standort erkannten CTR-Zone (Anker `[J-HEIGHT-GRID]`: `HGRID_COLORS`, `buildHeightGrid`, `updateHeightGridControl`, Toggle `#hgrid-toggle` + Legende). Näherung, verbindlich bleiben DiPUL/NfL; Toggle nur in DE-CTR 2/3/4. Vorgänger v26.08.111.0 — **Niederlande (`skycheck-nl`) + Portugal (`skycheck-pt`)** ergänzt: beide über das **EASA Common Repository** (ArcGIS-FeatureServer, clientseitige Vektor-Polygone, CORS-offen) nach dem IE-Adapter-Muster. NL = ED-318 (~162 Zonen, Ampel nach `restriction`), PT = KML-abgeleitetes Schema (~314 Zonen, Ampel nach `Restriction`, Typ aus `FolderPath`, ANAC-Kontakt). Country-Wiring (Detection, Namen, Bbox, Landmark, CC, Quellenleiste, Länder-Kachel) + `OVERLAY_ALL` auf `nl`/`pt` erweitert. Tschechien-ED-269 auf der Roadmap. Vorgänger v26.08.110.2 — Startseiten-Reihenfolge (Länder-Varianten zuerst); alle App-Versionen folgen `vYY.MM.major.subversion`.
...
|---|---|---|
| `const APP_VER = '26.08.112.0';` | ~3354 | **Zentrale Versionsvariable** im Schema `YY.MM.major.subversion` |
