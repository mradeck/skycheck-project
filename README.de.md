...
| **Luftraumkarte** | Länderspezifische Drohnen-Geozonen (DE: DiPUL · FR/AT: ED-269 · CH: geo.admin.ch · ES: ENAIRE · DK/IE: ArcGIS) — Flugverbotszonen, Kontrollzonen, Naturschutzgebiete; Suchradius zwischen 5 m und 100 m umschaltbar |
...
| v26.08.113.0 | 🇩🇪 **Höhengitter jetzt über die gesamte CTR (WFS-Neubau).** Die Ampel-Heatmap deckt nun die **ganze** DFS-Kontrollzone ab (wie die tschechische GRID) statt eines pin-zentrierten 3-km-Gitters und wird **pro CTR einmal berechnet und zwischengespeichert** — das Anklicken neuer Positionen innerhalb derselben CTR baut sie nicht mehr neu auf (behebt das zeitweise Verschwinden von Heatmap/Schaltfläche nach einigen Klicks). Neue Datenquelle: der **DiPUL-WFS** (`dipul:kontrollzonen`) liefert die exakten Sub-Zonen-Polygone (je `U_CONTROL_ZONE_1…4` mit MSL-Untergrenze); jede Zelle wird per Punkt-in-Polygon ihrer **korrekten** Sub-Zone zugeordnet, das Gelände (Open-Meteo) abgerufen und über `computeCtrFlightInfo` eingefärbt — die Stufen 2/3/4 sind damit echt statt einer einzigen erkannten Zone für alles. Der Umschalter bleibt in ganz Deutschland sichtbar; verbindlich bleibt DiPUL / NfL. |
...
| v26.08.110.0 | **GPS2UTM-Integration und gemeinsames Versionsformat.** Die Startseite verlinkt GPS2UTM mit dem gemeinsamen weiß/rot/cyanfarbenen Schriftzug. SkyCheck nutzt nun `vYY.MM.major.subversion`, vollständig angezeigt im App-Kopf, im Footer und im Browser-Tab. |
| v1.09 | **Ausklappbare Kartensteuerung auf jeder Bildschirmgröße.** Die kompakte einzeilige Werkzeugleiste und die eingeklappten Wetter-/Höhen-/Koordinatendetails ersetzen nun auch die früheren Desktop-Overlays, sodass Splitscreen- und Breitbild-Vorschauen dieselbe Oberfläche wie Smartphones verwenden. |
| v1.08 | **Kompakte mobile Karten-Overlays.** Die Kartenwerkzeugleiste ist auf Smartphones auf eine Zeile reduziert; die Suchradien 5/100/200 m und der 1,5-km-Alarmkreis öffnen sich bei Bedarf. Die Zonenliste bleibt sichtbar, während Wetter, Gelände und Koordinaten einklappbar sind; Adress-Popups verdoppeln die GPS-Koordinaten nicht mehr. |
| v1.07 | **GPS-Koordinatenumrechnung und Höhenbezug.** WGS84/ETRS89-Positionen zeigen nun sofort UTM sowie ausklappbare Gauß-Krüger- und GCG2016/DHHN2016-Details. `coordinates.html` ist ein mobiler Live-GPS-Umrechner mit dem offiziellen Kontrollpunkt Lübben als eingebauter numerischer Kontrolle. |
...
Deutschland, Frankreich, Österreich, Schweiz, Spanien, Dänemark & Irland · Betrieb in VLOS · Keine Haftung für Vollständigkeit oder Richtigkeit der angezeigten Daten. Die Nutzung der App ersetzt keine erforderliche behördliche Genehmigung. SkyCheck ist eine **Orientierungshilfe** — die gesetzlich vorgeschriebene Genehmigung und finale Luftraumfreigabe erfolgen über die zuständigen nationalen Portale (z. B. **DFS Aviation Services** für DE, **Austro Control Dronespace** für AT, **skyguide** für CH).

Datenquellen unterliegen ihren jeweiligen Lizenzen (DWD Open Data, GFZ CC BY 4.0, Airplanes.live Fair Use, NOAA Public Domain, DiPUL, Austro Control, BAZL / swisstopo geo.admin.ch, ENAIRE, Trafikstyrelsen, EASA).
