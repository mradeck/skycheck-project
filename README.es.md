...
| v26.08.114.0 | 🇩🇪 **Rejilla de alturas sobre teselas de elevación DEM + robustez del chequeo.** La rejilla lee ahora el terreno desde **teselas de elevación DEM** (Terrarium/AWS Open Data, ~25 m/px) en vez de muchas consultas puntuales de Open-Meteo: unas pocas teselas PNG dan el terreno a resolución completa de toda la CTR → relieves naturales, el **Domberg ya es visible**, cálculo ~1,6 s, y el cursor es **preciso al instante** (sin esperas). El mapa de calor es un único `L.imageOverlay` nítido (sin costuras/artefactos) y la escala tipo semáforo está calibrada a los topes de la DFS (25 m naranja, 45 m amarillo). **Robustez** (corrige errores en producción de 113.2): meteo/Kp/tráfico son no fatales cada uno, una fuente caída ya no vacía todo el chequeo; `fetchWeather` usa una ventana de 6 días + tiempo de espera (la de 7 días expiraba en BrightSky); se corrige una errata `$alarm`. |
| v26.08.113.2 | 🇩🇪 **Rejilla de alturas afinada.** Celdas finas de ~300 m con renderizador de lienzo (antes ~940 m), que se ciñen a los límites redondeados de la CTR — las celdas gruesas se salían de los bordes y podían mostrar un valor verde de zona 4 en un punto de zona 3. El terreno ahora se interpola bilinealmente desde una malla de apoyo gruesa, con reintento por bloque y relleno de huecos por vecino más cercano, de modo que una petición de altitud limitada ya no vacía media CTR. El cursor ahora calcula la **altura máxima en la posición exacta** (un valor interpolado inmediato y luego la altitud Open-Meteo precisa en ese punto) — notablemente menor en colinas como el Domberg — mostrada como una etiqueta oscura bajo el cursor para no solapar el tooltip de zona de DiPUL. Móvil: el panel de geozonas se coloca siempre bajo la barra de botones (de varias filas), sin solapamientos (S24 Ultra y menores). La fórmula de altura ya era correcta. |
| v26.08.113.0 | 🇩🇪 **La rejilla de alturas ahora cubre toda la CTR (rediseño WFS).** El mapa de calor tipo semáforo abarca ahora la **totalidad** de la zona de control de la DFS (como la GRID checa) en lugar de una rejilla de 3 km centrada en el marcador, y se **calcula una sola vez por CTR y se almacena en caché**: hacer clic en nuevas posiciones dentro de la misma CTR ya no la reconstruye (corrige la desaparición intermitente del mapa/botón tras varios clics). Nueva fuente: el **WFS de DiPUL** (`dipul:kontrollzonen`) proporciona los polígonos exactos de las subzonas (cada `U_CONTROL_ZONE_1…4` con su límite inferior MSL); cada celda se asigna a su subzona **correcta** mediante punto-en-polígono, se obtiene el terreno (Open-Meteo) y se colorea con `computeCtrFlightInfo`, de modo que los escalones 2/3/4 son reales en vez de una única zona detectada aplicada a todo. El conmutador permanece visible en toda Alemania; el carácter vinculante sigue siendo DiPUL / NfL. |
...
| v26.08.110.0 | **Integración de GPS2UTM y formato de versión compartido.** La página de inicio enlaza con GPS2UTM mediante el logotipo tipográfico blanco/rojo/cian compartido. SkyCheck utiliza ahora `vYY.MM.major.subversion`, mostrado completo en el encabezado de la aplicación, el pie de página y la pestaña del navegador. |
| v1.09 | **Controles de mapa contraíbles en cualquier tamaño de pantalla.** La barra de herramientas compacta de una sola fila y los detalles contraídos de meteorología, elevación y coordenadas reemplazan ahora también las antiguas superposiciones de escritorio, de modo que las vistas en pantalla dividida y las previsualizaciones anchas utilizan la misma interfaz que los teléfonos. |
| v1.08 | **Superposiciones de mapa compactas para móvil.** La barra de herramientas del mapa se reduce a una sola fila en los teléfonos; los radios de búsqueda de 5/100/200 m y el círculo de alarma de 1,5 km se abren bajo demanda. La lista de zonas permanece visible mientras que la meteorología, el terreno y las coordenadas son contraíbles; los cuadros emergentes de direcciones ya no duplican las coordenadas GPS. |
| v1.07 | **Conversión de coordenadas GPS y datum vertical.** Las posiciones WGS84/ETRS89 muestran ahora UTM de inmediato, además de detalles ampliables de Gauss-Krüger y GCG2016/DHHN2016. `coordinates.html` es un conversor de GPS en vivo para móvil, con el punto de control oficial de Lübben integrado como verificación numérica. |
...
| v1.03 | **Rendimiento en Francia.** Las cuatro capas de contexto de OSM ya no se cargan como archivos completos de país, sino agrupadas por teselas de vista de 2° — para París esto reduce la transferencia de ~4,1 MB a ~0,25 MB gzip. La función de zonas francesas también lee solo las teselas espaciales relevantes en un arranque en frío, en lugar del conjunto de datos completo de 8,8 MB. |
| v1.02 | **Rendimiento del análisis puntual.** Sin cargador de pantalla completa que bloquee, un círculo de análisis animado, consultas en paralelo, cachés espaciales de corta duración, renderizado en canvas y una caché de navegador/CDN mejorada. |
| v0.98 | **El título de la pestaña del navegador ahora sigue el idioma de la interfaz.** La etiqueta `<title>` era una cadena alemana estática nunca conectada a la i18n — todos los sitios que no son DE mostraban un título de pestaña en alemán. Ahora `document.title` se establece a partir de una clave i18n `docTitle` en `applyLang()` — se actualiza al cargar (según el idioma predeterminado del país) y en cada cambio de idioma, en los cinco idiomas. |
| v0.97 | Capas de contexto ampliadas a CH, ES, DK, IE y FR. Las superposiciones de Austria de la v0.96 (áreas protegidas, autopistas, líneas eléctricas, ferrocarriles) existen ahora para todos los países que no son DE (DE ya las tiene a través de DiPUL). El cliente se ha generalizado — CONTEXT_META + CONTEXT_COUNTRIES, un archivo de datos por país (data/<cc>-<layer>.json); el grupo de botones de activación aparece automáticamente para cualquier país listado. El generador es ahora una única herramienta reutilizable (scripts/gen-context.mjs + un robusto controlador fetch-context.sh con reintentos de curl y reanudación) que une los segmentos de OSM en polilíneas largas antes de simplificar, lo que redujo notablemente el tamaño de los países grandes (autopistas de FR de 9 a 3,5 MB, ES de 6 a 2,9 MB antes de gzip). Francia se recorta a un cuadro delimitador metropolitano (el área ISO arrastraría los territorios de ultramar). Añadir un país en el futuro = generar los datos + agregar el código a CONTEXT_COUNTRIES (véase scripts/README.md). |
| v0.96 | 🇦🇹 Capas de contexto para Austria. skycheck-at ahora puede superponer cuatro capas adicionales, activables de forma individual — áreas protegidas (parques nacionales, Natura 2000, reservas naturales, áreas de protección del paisaje, parques naturales, reservas de la biosfera, elementos protegidos del paisaje, monumentos naturales), autopistas, líneas eléctricas de alta tensión y ferrocarriles principales. Obtenidas de OpenStreetMap (ODbL) como instantáneas GeoJSON estáticas y presimplificadas en data/at-*.json (generadores en scripts/), cargadas de forma diferida al activarlas y dibujadas como superposiciones vectoriales de color en el lado del cliente. Son contexto informativo (orientación para mantener distancias) — la comprobación vinculante de zonas sigue siendo la de Austro Control / dronespace.at, por lo que no intervienen en la evaluación del estado de vuelo. Inspiradas en drohnenkarte.at, que utiliza las mismas fuentes abiertas. |
| v0.95 | Hotfix para v0.94: el reemplazo global de-DE→_locale() había alcanzado también la propia definición del mapa _LOCALES (de: _locale()), que leía _LOCALES mientras este aún se estaba inicializando — un error de zona muerta temporal (temporal dead zone) que impedía que toda la aplicación arrancara. Se restauró el literal 'de-DE'. (node --check no detecta errores de sintaxis; se trataba de una regresión solo en tiempo de ejecución, descubierta en la prueba de humo en vivo.) |
| v0.94 | Auditoría completa de i18n — se acabaron las filtraciones de alemán a otros idiomas. Varias secciones renderizadas dinámicamente (análisis de punto de rocío/engelamiento, etiquetas METAR/TAF, base de nubes estimada, mensajes de carga/error, ventanas emergentes de los marcadores de aeronaves, alertas y la marca de tiempo «Actualizado») estaban codificadas de forma fija en alemán o solo volvían a renderizar sus etiquetas estáticas al cambiar de idioma — de modo que los usuarios de inglés/francés seguían viendo alemán. Ahora todas las cadenas visibles para el usuario pasan por la tabla de i18n (16 claves nuevas × 5 idiomas), un asistente _locale() gobierna todo el formato de fecha/hora (antes fijado en de-DE) y al cambiar de idioma se vuelven a renderizar todas las secciones dinámicas de resultados, no solo las etiquetas estáticas data-i18n |
