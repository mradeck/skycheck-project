...
**Aktuell:** v26.08.113.2 — **Höhengitter verfeinert (Auflösung, Präzisions-Hover, Mobile-Layout).** Feines ~300-m-Raster (`HGRID_CELL_M`, Canvas-Renderer) statt ~940-m-Kacheln → scharfe, an die runden CTR-Grenzen geschmiegte Zellen (die groben Kacheln überlappten die Zonenränder und zeigten an einem Zone-3-Pin fälschlich grüne Zone-4-Werte). Gelände jetzt aus grobem Stützgitter **bilinear interpoliert** (`fillTerrainGaps` + Chunk-Retry in `fetchElevations` → ein von Open-Meteo gedrosselter Block leert keine halbe CTR mehr). **Präzisions-Hover** (`onHgridHover`): rechnet die max. Flughöhe an der EXAKTEN Cursorposition — Sofortwert interpoliert, nach kurzem Ruhen exakte Open-Meteo-Höhe am Punkt (debounced/gecacht) → auf Hügeln (z. B. Domberg) sichtbar niedriger; Tooltip als dunkler Chip **unter** dem Cursor, damit er den DiPUL-Zonen-Tooltip nicht überlagert. **Mobile:** `positionMapStatus()` setzt das Geozonen-Panel per gemessener Steuerungshöhe stets unter die (mehrzeilige) Buttonreihe (kein Überlappen mehr, S24 Ultra & kleiner). Die Höhenformel (`computeCtrFlightInfo`) blieb unverändert korrekt (= NfL 2026-1-3960). Vorgänger v26.08.113.0 — **DE-Höhengitter über die GESAMTE CTR (WFS-Redesign):** Ampel-Heatmap der ganzen DFS-CTR (wie das tschechische GRID) via **DiPUL-WFS** (`dipul:kontrollzonen`, `DIPUL_WFS_URL`) → exakte Sub-Zonen-Polygone (`U_CONTROL_ZONE_1…4`, `lower_limit_altitude` MSL), Punkt-in-Polygon-Klassifizierung je Zelle, pro CTR einmal berechnet und gecacht. Vorgänger v26.08.112.0 — erstes DE-Höhengitter (pin-zentriertes 13×13-Hybrid-Raster). Vorgänger v26.08.111.0 — **Niederlande (`skycheck-nl`) + Portugal (`skycheck-pt`)** ergänzt: beide über das **EASA Common Repository** (ArcGIS-FeatureServer, clientseitige Vektor-Polygone, CORS-offen) nach dem IE-Adapter-Muster. NL = ED-318 (~162 Zonen, Ampel nach `restriction`), PT = KML-abgeleitetes Schema (~314 Zonen, Ampel nach `Restriction`, Typ aus `FolderPath`, ANAC-Kontakt). Country-Wiring (Detection, Namen, Bbox, Landmark, CC, Quellenleiste, Länder-Kachel) + `OVERLAY_ALL` auf `nl`/`pt` erweitert. Tschechien-ED-269 auf der Roadmap. Vorgänger v26.08.110.2 — Startseiten-Reihenfolge (Länder-Varianten zuerst); alle App-Versionen folgen `vYY.MM.major.subversion`.
...
|---|---|---|
| `const APP_VER = '26.08.113.2';` | ~3354 | **Zentrale Versionsvariable** im Schema `YY.MM.major.subversion` |
