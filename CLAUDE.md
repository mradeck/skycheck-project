...
**Aktuell:** v26.08.113.0 — **DE-Höhengitter über die GESAMTE CTR (WFS-Redesign).** Die Ampel-Heatmap deckt jetzt die ganze DFS-CTR ab (wie das tschechische GRID) statt eines pin-zentrierten 3-km-Rasters, und wird **pro CTR nur einmal berechnet und gecacht** — Positionsklicks innerhalb derselben CTR bauen nichts mehr neu (behebt das inkonsistente Verschwinden von Heatmap/Button). Neue Datenquelle: **DiPUL-WFS** (`dipul:kontrollzonen`, `DIPUL_WFS_URL`) liefert die exakten Sub-Zonen-Polygone (je `U_CONTROL_ZONE_1…4` mit `lower_limit_altitude` MSL); jede Rasterzelle wird per Punkt-in-Polygon (`fetchCtrSubzones`/`pointInFeature`) ihrer **korrekten** Sub-Zone zugeordnet, das Gelände (Open-Meteo) geholt und via `computeCtrFlightInfo` eingefärbt — dadurch echte 2/3/4-Staffelung statt einer einzigen Zone für alle Zellen. Toggle `#hgrid-toggle` in DE stets sichtbar; `updateHeightGridControl` baut nur neu, wenn der Pin die gecachte CTR verlässt. Näherung, verbindlich bleiben DiPUL/NfL. Vorgänger v26.08.112.0 — erstes DE-Höhengitter (pin-zentriertes 13×13-Hybrid-Raster). Vorgänger v26.08.111.0 — **Niederlande (`skycheck-nl`) + Portugal (`skycheck-pt`)** ergänzt: beide über das **EASA Common Repository** (ArcGIS-FeatureServer, clientseitige Vektor-Polygone, CORS-offen) nach dem IE-Adapter-Muster. NL = ED-318 (~162 Zonen, Ampel nach `restriction`), PT = KML-abgeleitetes Schema (~314 Zonen, Ampel nach `Restriction`, Typ aus `FolderPath`, ANAC-Kontakt). Country-Wiring (Detection, Namen, Bbox, Landmark, CC, Quellenleiste, Länder-Kachel) + `OVERLAY_ALL` auf `nl`/`pt` erweitert. Tschechien-ED-269 auf der Roadmap. Vorgänger v26.08.110.2 — Startseiten-Reihenfolge (Länder-Varianten zuerst); alle App-Versionen folgen `vYY.MM.major.subversion`.
...
|---|---|---|
| `const APP_VER = '26.08.113.0';` | ~3354 | **Zentrale Versionsvariable** im Schema `YY.MM.major.subversion` |
